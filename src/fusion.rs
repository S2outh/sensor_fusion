...
use crate::math_utils::{
    ...
    measurement_function, measurement_jacobian, normalize_quaternion,
    state_transition, state_transition_jacobian,
};
use crate::run_utils::confirm;
...
pub fn init_ekf(data: &mut FlightData) -> RocketEKF {
    let start_idx = (0..data.lat.len())
        ...
        .expect("Inital position can not be correct!");
    ...
    let ecef_ref = Vector3::from([data.x[start_idx], data.y[start_idx], data.z[start_idx]]);
